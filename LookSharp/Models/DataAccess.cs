...
namespace LookSharp.Models
{
    public class DataAccess
    {
        ...
        public async Task AddPassItOn(string passitCode, string mobileNo, string campaignId, string prize, string network = null)
        {
            ...
            if (network == "+23324" || network == "+23354" || network == "+23355")
            {
                passItOn.ServiceCode = "mtn-money";
            }

            if (network == "+23320" || network == "+23350")
            {
                passItOn.ServiceCode = "vodafone-cash";
            }

            if (network == "+23327" || network == "+23357")
            {
        ...
        #region LookSharpTasks
        public async Task<bool> LookSharp(string id, string choice)
        {
            var builder = Builders<Campaign>.Filter;
            var filter = builder.Eq("_id", ObjectId.Parse(id)) & builder.Eq("WinCode", choice) & builder.Eq("CampaignStatus", true);
            var query =  await _dbContext.Campaigns.Find(filter).FirstOrDefaultAsync();

            if (query != null)
            {
                //Deactivate Campaign Immediately
                var update = Builders<Campaign>.Update.Set("CampaignStatus",false).CurrentDate("TimeUpdated");

                UpdateResult actionResult = await _dbContext.Campaigns.UpdateOneAsync(filter, update);
